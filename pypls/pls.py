...
def _permute_and_calc_singular_values(X, Y, X_saliences, Y_saliences, singular_values_samples, perm_i, n_components, procrustes=False):
    if len(X) < len(Y):
    ...
    Y_saliences_perm, singular_values_perm, X_saliences_perm = randomized_svd(covariance_perm, n_components, n_iter=5)
...
def _boostrap(X, Y, X_saliences, Y_saliences, X_saliences_bootstraps, Y_saliences_bootstraps, bootstrap_i, n_components):
    ...
    Y_saliences_boot, _, X_saliences_boot = randomized_svd(covariance_boot, n_components, n_iter=5)
...
def fit_pls(X, Y, n_components, scale=True):
    #scaling
    if scale:
    ...
    Y_saliences, singular_values, X_saliences = randomized_svd(covariance, n_components, n_iter=5)
...
def permutation_test(X_scaled, Y_scaled, X_saliences, Y_saliences, singular_values, inertia, n_perm, verbose=False):
    ...
    for perm_i in range(n_perm):
        _permute_and_calc_singular_values(X_scaled, Y_scaled, X_saliences, Y_saliences, singular_values_samples, perm_i, n_components)
        if verbose:
            my_perc.update()
    if verbose:
        print my_perc
